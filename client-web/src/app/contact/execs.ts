import { ExecInterface } from './exec.interface';

export const execsConst = [
  {
    position: 'président',
    fullname: 'Nicholas Cantin',
    email: 'president@ageei.org',
    picture: 'assets/contact/execs/president.jpg',
    github: 'Cantinoo',
    linkedin: 'nicholas-cantin-157aaa285',
  },
  {
    position: 'secrétaire',
    fullname: 'Mohamed Fayçal Sakho',
    email: 'secretaire@ageei.org',
    picture: 'assets/contact/execs/blank_m.png',
  },
  {
    position: 'trésorier',
    fullname: 'Émile Kolani',
    email: 'tresorier@ageei.org',
    picture: 'assets/contact/execs/tresorier.jpg',
    linkedin: 'émile-kolani-711720233',
  },
  {
    position: 'vp-interne',
    fullname: 'Olivier Provost',
    email: 'interne@ageei.org',
    picture: 'assets/contact/execs/interne.jpg',
    github: 'FouDuRoy',
    linkedin: 'olivierprovost1',
  },
  {
    position: 'vp-externe',
    fullname: 'Badr Malouli',
    email: 'externe@ageei.org',
    picture: 'assets/contact/execs/externe.jpg',
    github: 'bmalouli',
    linkedin: 'badrmalouli',
  },
  {
    position: 'vp-loisir',
    fullname: 'Setayesh Abbasi Moghadam',
    email: 'loisir@ageei.org',
    picture: 'assets/contact/execs/blank_f.png',
    linkedin: 'setayesh-abbasi-moghadam',
  },
  {
    position: 'vp-techno',
    fullname: 'George Fam',
    email: 'technologie@ageei.org',
    picture: 'assets/contact/execs/technologie.png',
    github: 'George-Fam',
    linkedin: 'george-fam',
  },
  {
    position: 'vp-compétition',
    fullname: 'Elouan Hildgen-Poirier',
    email: 'competition@ageei.org',
    picture: 'assets/contact/execs/competition.jpg',
    github: 'CmD0',
    linkedin: 'elouan-hildgen-poirier-2625ba20b',
  },
  {
    position: 'officier-premiere-annee',
    fullname: 'Vacant',
    email: 'premiere_annee@ageei.org',
    picture: 'assets/contact/execs/blank_m.png',
  },
] as ExecInterface[];
